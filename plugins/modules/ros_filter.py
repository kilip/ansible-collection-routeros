...
"""
The module file for ros_filter
"""
...
options:
    state:
        ...
        default: merged
        description:
            - I(merged) M(ros_filter) will update existing C(/ip firewall filter) configuration, or create new C(/ip firewall filter) when resource not found
            - I(replaced) M(ros_filter) will restore existing C(/ip firewall filter) configuration to its default value, then update existing resource with the new configuration. If the resource C(/ip firewall filter) not found, M(ros_filter) will create resource in C(/ip firewall filter)
            - I(overridden) M(ros_filter) will remove any resource in C(/ip firewall filter) first, and then create new C(/ip firewall filter) resources.
            - I(deleted) M({module}) when found module will delete C(/ip firewall filter)
    config:
        description: A dictionary for L(ros_filter)
        type: list
        elements: dict
        suboptions:
            action:
                ...
                description: |
                    ...
                    - add-dst-to-address-list - add destination address to L( address list,/wiki/Manual:IP/Firewall/Address_list) specified by C(address-list) parameter
                    - add-src-to-address-list - add source address to L( address list,/wiki/Manual:IP/Firewall/Address_list) specified by C(address-list) parameter
                    - drop - silently drop the packet
                    - fasttrack-connection - process packets from a connection using FastPath by enabling L( FastTrack,/wiki/Manual:IP/Fasttrack) for the connection
            ...
            connection_rate:
                type: int
                description: |
                    Connection Rate is a firewall matcher that allow to capture traffic based on present speed of the connection. L( Read more &gt;&gt;,/wiki/Manual:Connection_Rate)
            connection_state:
                type: str
                choices:
                    - established
                ...
                description: |
                    ...
                    - untracked - packet which was set to bypass connection tracking in firewall L( RAW,/wiki/Manual:IP/Firewall/Raw) tables.
            connection_type:
                type: str
                choices:
                    - ftp
                    ...
                    - tftp
                description: |
                    Matches packets from related connections based on information from their connection tracking helpers. A relevant connection helper must be enabled under L( /ip firewall service-port,/wiki/Manual:IP/Services)
            content:
                type: str
                description: |
            ...
            dst_address_list:
                type: str
                description: |
                    Matches destination address of a packet against user-defined L( address list,/wiki/Manual:IP/Firewall/Address_list)
            dst_address_type:
                type: str
                choices:
                    - unicast
            ...
            in_bridge_port_list:
                type: str
                description: |
                    Set of interfaces defined in L( interface list,/wiki/Manual:Interface/List). Works the same as in-bridge-port
            in_interface:
                type: str
                description: |
            ...
            ingress_priority:
                type: int
                description: |
                    Matches the priority of an ingress packet. Priority may be derived from VLAN, WMM, DSCP or MPLS EXP bit. L( read more»,/wiki/WMM)
            ipsec_policy:
                type: str
                choices:
                    - in
            ...
            out_bridge_port_list:
                type: str
                description: |
                    Set of interfaces defined in L( interface list,/wiki/Manual:Interface/List). Works the same as out-bridge-port
            out_interface:
                type: str
                description: |
            ...
            priority:
                type: int
                description: |
                    Matches packets priority after a new priority has been set. Priority may be derived from VLAN, WMM, DSCP, MPLS EXP bit or from priority that has been set using the set-priority action. L( Read more &gt;&gt;,/wiki/WMM)
            protocol:
                type: str
                default: tcp
                description: |
            ...
            reject_with:
                ...
                description: |
                    Specifies ICMP error to be sent back if packet is rejected. Applicable if C(action=reject)
            routing_table:
                type: str
                description: |
                    Matches packets which destination address is resolved in specific a routing table. More details can be found in the L( Routing Table Matcher,/wiki/Manual:Routing_Table_Matcher) page
            routing_mark:
                type: str
                description: |
            ...
            src_address_list:
                type: str
                description: |
                    Matches source address of a packet against user-defined L( address list,/wiki/Manual:IP/Firewall/Address_list)
            src_address_type:
                type: str
                choices:
                    - unicast
            ...
            tls_host:
                type: str
                description: |
                    Allows to match https traffic based on TLS SNI hostname. Accepts L(GLOB syntax,https://en.wikipedia.org/wiki/Glob_(programming)) for wildcard matching. Note that matcher will not be able to match hostname if TLS handshake frame is fragmented into multiple TCP segments (packets).
            ttl:
                type: int
                description: |
...
from ..module_utils.resources.firewall.filter import FilterResource
from ..module_utils.config.resource import ResourceConfig


def main():
    module = AnsibleModule(argument_spec=FilterResource.argument_spec)
    result = ResourceConfig(module, FilterResource).execute_module()
