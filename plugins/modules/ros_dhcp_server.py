...
options:
    state:
        ...
        default: merged
        description:
            - I(merged) M(ros_dhcp_server) will update existing C(/ip dhcp-server) configuration, or create new C(/ip dhcp-server) when resource not found
            - I(replaced) M(ros_dhcp_server) will restore existing C(/ip dhcp-server) configuration to its default value, then update existing resource with the new configuration. If the resource C(/ip dhcp-server) not found, M(ros_dhcp_server) will create resource in C(/ip dhcp-server)
            - I(overridden) M(ros_dhcp_server) will remove any resource in C(/ip dhcp-server) first, and then create new C(/ip dhcp-server) resources.
            - I(deleted) M({module}) when found module will delete C(/ip dhcp-server)
    config:
