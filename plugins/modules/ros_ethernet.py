...
options:
    state:
        ...
        default: merged
        description:
            - I(merged) M(ros_ethernet) will update existing C(/interface ethernet) configuration, or create new C(/interface ethernet) when resource not found
            - I(replaced) M(ros_ethernet) will restore existing C(/interface ethernet) configuration to its default value, then update existing resource with the new configuration. If the resource C(/interface ethernet) not found, M(ros_ethernet) will create resource in C(/interface ethernet)
            - I(overridden) M(ros_ethernet) will remove any resource in C(/interface ethernet) first, and then create new C(/interface ethernet) resources.
            - I(deleted) M({module}) when found module will delete C(/interface ethernet)
    config:
