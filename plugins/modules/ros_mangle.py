...
"""
The module file for ros_mangle
"""
...
options:
    state:
        ...
        default: merged
        description:
            - I(merged) M(ros_mangle) will update existing C(/ip firewall mangle) configuration, or create new C(/ip firewall mangle) when resource not found
            - I(replaced) M(ros_mangle) will restore existing C(/ip firewall mangle) configuration to its default value, then update existing resource with the new configuration. If the resource C(/ip firewall mangle) not found, M(ros_mangle) will create resource in C(/ip firewall mangle)
            - I(overridden) M(ros_mangle) will remove any resource in C(/ip firewall mangle) first, and then create new C(/ip firewall mangle) resources.
            - I(deleted) M({module}) when found module will delete C(/ip firewall mangle)
    config:
        description: A dictionary for L(ros_mangle)
        type: list
        elements: dict
        suboptions:
            action:
...
from ..module_utils.resources.firewall.mangle import MangleResource
from ..module_utils.config.resource import ResourceConfig


def main():
    module = AnsibleModule(argument_spec=MangleResource.argument_spec)
    result = ResourceConfig(module, MangleResource).execute_module()
