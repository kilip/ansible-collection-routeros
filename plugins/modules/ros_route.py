...
options:
    state:
        ...
        default: merged
        description:
            - I(merged) M(ros_route) will update existing C(/ip route) configuration, or create new C(/ip route) when resource not found
            - I(replaced) M(ros_route) will restore existing C(/ip route) configuration to its default value, then update existing resource with the new configuration. If the resource C(/ip route) not found, M(ros_route) will create resource in C(/ip route)
            - I(overridden) M(ros_route) will remove any resource in C(/ip route) first, and then create new C(/ip route) resources.
            - I(deleted) M({module}) when found module will delete C(/ip route)
    config:
