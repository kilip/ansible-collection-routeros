...
options:
    state:
        ...
        default: merged
        description:
            - I(merged) M(ros_firewall_nat) will update existing C(/ip firewall nat) configuration, or create new C(/ip firewall nat) when resource not found
            - I(replaced) M(ros_firewall_nat) will restore existing C(/ip firewall nat) configuration to its default value, then update existing resource with the new configuration. If the resource C(/ip firewall nat) not found, M(ros_firewall_nat) will create resource in C(/ip firewall nat)
            - I(overridden) M(ros_firewall_nat) will remove any resource in C(/ip firewall nat) first, and then create new C(/ip firewall nat) resources.
