...
options:
    state:
        ...
        default: merged
        description:
            - I(merged) M(ros_bridge) will update existing C(/interface bridge) configuration, or create new C(/interface bridge) when resource not found
            - I(replaced) M(ros_bridge) will restore existing C(/interface bridge) configuration to its default value, then update existing resource with the new configuration. If the resource C(/interface bridge) not found, M(ros_bridge) will create resource in C(/interface bridge)
            - I(overridden) M(ros_bridge) will remove any resource in C(/interface bridge) first, and then create new C(/interface bridge) resources.
            - I(deleted) M({module}) when found module will delete C(/interface bridge)
    config:
        ...
        suboptions:
            ...
            fast_forward:
                ...
                choices:
                    ...
                    - no
                description: |
                    Special and faster case of L(FastPath,https://wiki.mikrotik.com/wiki/Manual:Fast_Path) section.
            forward_delay:
                type: str
                default: 00:00:15
                description: |
