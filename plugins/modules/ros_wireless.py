...
options:
  state:
    ...
    default: merged
  config:
    description: A dictionary of `/interface wireless` parameters
    type: list
    elements: dict
    suboptions:
        ...
        allow_sharedkey:
          ...
          default: "no"
          description: |
            Allow WEP Shared Key clients to connect. Note that no authentication is done for
            these clients (WEP Shared keys are not compared to anything) - they are just
        ...
        ampdu_priorities:
          ...
          default: [0]
          description: |
            Frame priorities for which AMPDU sending (aggregating frames and sending using
            block acknowledgment) should get negotiated and used. Using AMPDUs will increase
            throughput, but may increase latency, therefore, may not be desirable for
            real-time traffic (voice, video). Due to this, by default AMPDUs are enabled
        ...
        amsdu_limit:
          ...
          default: 8192
          description: |
            Max AMSDU that device is allowed to prepare when negotiated. AMSDU aggregation
            may significantly increase throughput especially for small frames, but may
            increase latency in case of packet loss due to retransmission of aggregated
            frame. Sending and receiving AMSDUs will also increase CPU usage.
        ...
        antenna_gain:
          ...
          description: |
            Antenna gain in dBi, used to calculate maximum transmit power according to
            **country** regulations.
        ...
        area:
          ...
          description: |
            Identifies group of wireless networks. This value is announced by AP, and can be
            matched in L(connect-list,#Connect_List) by **area-prefix**. This is a
        ...
        basic_rates_ag:
          ...
          description: |
            ...
            5ghz-5mhz, 5ghz-turbo, 2.4ghz-b/g, 2.4ghz-onlyg, 2ghz-10mhz, 2ghz-5mhz and
            2.4ghz-g-turbo bands.
        ...
        basic_rates_b:
          ...
          description: |
            ...
            Client will connect to AP only if it supports all basic rates announced by the
            AP. AP will establish WDS link only if it supports all basic rates of the other
            AP.
        ...
        burst_time:
          ...
          description: |
            Time in microseconds which will be used to send data without stopping. Note that
            no other wireless cards in that network will be able to transmit data during
        ...
        channel_width:
          ...
          default: "20mhz"
          description: |
            Use of extension channels (e.g. Ce, eC etc) allows additional 20MHz extension
            channels and if it should be located below or above the control (main) channel.
            Extension channel allows 802.11n devices to use up to 40MHz (802.11ac up to
        ...
        compression:
          ...
          default: "no"
          description: |
            Setting this property to *yes* will allow the use of the hardware compression.
            Wireless interface must have support for hardware compression. Connections with
            devices that do not use compression will still work.
        ...
        default_ap_tx_limit:
          ...
          description: |
            ...
            the L(access-list,#Access_List). 0 means no limit.
        ...
        default_authentication:
          ...
          description: |
            ...
            match any entry in the L(access-list,#Access_List). For station mode, this is the value of **connect** for APs that do not match any entry in the L(connect-list,#Connect_List)

        default_client_tx_limit:
          type: int

          description: |
            This is the value of **client-tx-limit** for clients that do not match any entry
            in the L(access-list,#Access_List). 0 means no limit
        ...
        disable_running_check:
          ...
          description: |
            ...
            **no'**, the router determines whether the card is up and running - for AP one
            or more clients have to be registered to it, for station, it should be connected
            to an AP.
        ...
        disconnect_timeout:
          ...
          default: "3s"
          description: |
            This interval is measured from third sending failure on the lowest data rate. At
            this point 3 * (**hw-retries** + 1) frame transmits on the lowest data rate had
        ...
        distance:
          ...
          description: |
            How long to wait for confirmation of unicast frames (**ACKs**) before
            considering transmission unsuccessful, or in short **ACK-Timeout**. Distance
            value has these behaviors:
            - *Dynamic* - causes AP to detect and use the smallest timeout that works with
              all connected clients.
            - *Indoor* - uses the default ACK timeout value that the hardware chip
              manufacturer has set.
            - *Number* - uses the input value in formula: ACK-timeout = ((**distance** *
              1000) + 299) / 300 us;
        ...
        frame_lifetime:
          ...
          description: |
            Discard frames that have been queued for sending longer than **frame-lifetime**.
            By default, when value of this property is 0, frames are discarded only after
        ...
        frequency:
          ...
          description: |
            ...
            Allowed values depend on the selected band, and are restricted by **country**
        ...
        frequency_mode:
          ...
          description: |
            ...
            - *superchannel* - Conformance Testing Mode. Allow all channels supported by the
              card.
            List of available channels for each band can be seen in **/interface wireless
            info allowed-channels**. This mode allows you to test wireless channels outside
            the default scan-list and/or regulatory domain. This mode should only be used in
            controlled environments, or if you have special permission to use it in your
            region. Before v4.3 this was called Custom Frequency Upgrade, or Superchannel.
            Since RouterOS v4.3 this mode is available without special key upgrades to all
            installations.
        ...
        frequency_offset:
          ...
          description: |
            ...
            frequency than is shown in RouterOS, in case a frequency converter is used in
            the card. So if your card works at 4000MHz but RouterOS shows 5000MHz, set
            offset to 1000MHz and it will be displayed correctly. The value is in MHz and
        ...
        hide_ssid:
          ...
          description: |
            ...
            This property has an effect only in AP mode. Setting it to *yes* can remove this
        ...
        hw_fragmentation_threshold:
          ...
          description: |
            Specifies maximum fragment size in bytes when transmitted over the wireless
            medium. 802.11 standard packet (MSDU in 802.11 terminologies) fragmentation
            allows packets to be fragmented before transmitting over a wireless medium to
            increase the probability of successful transmission (only fragments that did not
            transmit correctly are retransmitted). Note that transmission of a fragmented
            packet is less efficient than transmitting unfragmented packet because of
            protocol overhead and increased resource usage at both - transmitting and
            receiving party.
        ...
        hw_protection_mode:
          ...
          default: "none"
          description: |
            Frame protection support property L(C(read more >>),#Frame_protection_support_.28RTS.2FCTS.29)

        hw_protection_threshold:
          type: int

          description: |
            Frame protection support propertyL(C(read more >>),#Frame_protection_support_.28RTS.2FCTS.29)

        hw_retries:
          type: int
          default: 7
          description: |
            Number of times sending frame is retried without considering it a transmission
            failure. Data-rate is decreased upon failure and the frame is sent again. Three
            sequential failures on the lowest supported rate suspend transmission to this
            destination for the duration of **on-fail-retry-time**. After that, the frame is
            sent again. The frame is being retransmitted until transmission success, or
            until the client is disconnected after **disconnect-timeout**. The frame can be
        ...
        installation:
          ...
          default: "any"
          description: |
            Adjusts scan-list to use indoor, outdoor or all frequencies for the country that
            is set.
        ...
        mode:
          ...
          description: |
            ...
            Special modes:
            - *alignment-only* - Put the interface in a continuous transmit mode that is
              used for aiming the remote antenna.
            - *nstreme-dual-slave* - allow this interface to be used in nstreme-dual setup.
            MAC address translation in **pseudobridge** modes works by inspecting packets
            and building a table of corresponding IP and MAC addresses. All packets are sent
            to AP with the MAC address used by pseudobridge, and MAC addresses of received
            packets are restored from the address translation table. There is a single entry
            in the address translation table for all non-IP packets, hence more than one
            host in the bridged network cannot reliably use non-IP protocols. Note:
            Currently IPv6 doesn't work over Pseudobridge
        ...
        multicast_buffering:
          ...
          default: "enabled"
          description: |
            For a client that has power saving, buffer multicast packets until next beacon
            time. A client should wake up to receive a beacon, by receiving beacon it sees
            that there are multicast packets pending, and it should wait for multicast
            packets to be sent.
        ...
        noise_floor_threshold:
          ...
          default: "default"
          description: |
            For advanced use only, as it can badly affect the performance of the interface.
            It is possible to manually set noise floor threshold value. By default, it is
            dynamically calculated. This property also affects received signal strength.
        ...
        periodic_calibration:
          ...
          default: "default"
          description: |
            Setting *default* enables periodic calibration if L(info,#Info)
            **default-periodic-calibration** property is *enabled*. Value of that property
            depends on the type of wireless card. This property is only effective for cards
        ...
        preamble_mode:
          ...
          default: "both"
          description: |
            Short preamble mode is an option of 802.11b standard that reduces per-frame
            overhead.
            - On AP:
        ...
        proprietary_extensions:
          ...
          description: |
            ...
            management frames. This parameter controls how this information is included.
            - *pre-2.9.25* - This is older method. It can interoperate with newer versions
              of RouterOS. This method is incompatible with some clients, for example,
              Centrino based ones.
        ...
        radio_name:
          ...
          description: |
            Descriptive name of the device, that is shown in registration table entries on
            the remote devices. This is a proprietary extension.
        ...
        rate_set:
          ...
          description: |
            ...
            - *configured* - use values from **basic-rates**, **supported-rates**,
              **basic-mcs**, **mcs**. L(C(Read more >>),#Basic_and_MCS_Rate_table).
        ...
        rx_chains:
          ...
          default: [0]
          description: |
            Which antennas to use for receive. In current MikroTik routers, both RX and TX
        ...
        scan_list:
          ...
          description: |
            ...
            can be seen in L(info,#Info)). For default scan list in *5ghz* band channels
            are taken with 20MHz step, in *5ghz-turbo* band - with 40MHz step, for all other
            ...
            **scan-list**=*default,5200-5245,2412-2427* - This will use the default value of
            scan list for current band, and add to it supported frequencies from 5200-5245
            or 2412-2427 range.)
            Since RouterOS v6.0 with Winbox or Webfig, for inputting of multiple
            frequencies, add each frequency or range of frequencies into separate multiple
            scan-lists. Using a comma to separate frequencies is no longer supported in
            Winbox/Webfig since v6.0.
            Since RouterOS v6.35 (wireless-rep) scan-list support step feature where it is
            possible to manually specify the scan step. Example:
            **scan-list**=*5500-5600:20* will generate such scan-list values
        ...
        security_profile:
          ...
          default: "default"
          description: |
            Name of profile from L(security-profiles,#Security_Profiles)

        skip_dfs_channels:
          type: str
          default: "disabled"
          description: |
            These values are used to skip all DFS channels or specifically skip DFS CAC
        ...
        station_bridge_clone_mac:
          ...
          description: |
            ...
            *00:00:00:00:00:00*, station will initially use MAC address of the wireless
            interface.
            As soon as packet with MAC address of another device needs to be transmitted,
        ...
        station_roaming:
          ...
          default: "disabled"
          description: |
            Station Roaming feature is available only for 802.11 wireless protocol and only
            for station modes. L(C(Read more >>),#Station-Roaming)
        ...
        supported_rates_b:
          ...
          description: |
            ...
            devices. This property has effect only when value of **rate-set** is
            *configured*.
        ...
        tdma_period_size:
          ...
          default: 2
          description: |
            Specifies TDMA period in milliseconds. It could help on the longer distance
        ...
        tx_chains:
          ...
          default: [0]
          description: |
            Which antennas to use for transmitting. In current MikroTik routers, both RX and
            TX chain must be enabled, for the chain to be enabled.

        tx_power:
          type: int

          description: |
            For 802.11ac wireless interface it's total power but for 802.11a/b/g/n it's
            power per chain.
        ...
        tx_power_mode:
          ...
          description: |
            ...
            - all-rates-fixed - use same transmit power for all data rates. Can damage the
            ...
            - manual-table - define transmit power for each rate separately. Can damage the
              card if transmit power is set above rated value of the card for used rate.
            - card-rates - use transmit power calculated for each rate based on value of
              **tx-power** parameter. Legacy mode only compatible with currently discontinued
              products.
        ...
        update_stats_interval:
          ...
          description: |
            ...
            Access to L(registration-table,#Registration_Table) also triggers update of
            these values.
        ...
        wds_cost_range:
          ...
          default: 50
          description: |
            Bridge port cost of WDS links are automatically adjusted, depending on measured
            link throughput. Port cost is recalculated and adjusted every 5 seconds if it
            has changed by more than 10%, or if more than 20 seconds have passed since the
            last adjustment.
            Setting this property to 0 disables automatic cost adjustment.
            Automatic adjustment does not work for WDS links that are manually configured as
            a bridge port.
        ...
        wds_default_bridge:
          ...
          default: "none"
          description: |
            When WDS link is established and status of the wds interface becomes *running*,
            it will be added as a bridge port to the bridge interface specified by this
            property. When WDS link is lost, wds interface is removed from the bridge. If
            wds interface is already included in a bridge setup when WDS link becomes
        ...
        wds_ignore_ssid:
          ...
          default: "no"
          description: |
            By default, WDS link between two APs can be created only when they work on the
            same frequency and have the same SSID value. If this property is set to *yes*,
            then SSID of the remote AP will not be checked. This property has no effect on
            connections from clients in *station-wds* mode. It also does not work if
            **wds-mode** is *static-mesh* or *dynamic-mesh*.
        ...
        wds_mode:
          ...
          default: "disabled"
          description: |
            Controls how WDS links with other devices (APs and clients in *station-wds*
            ...
            *-mesh* modes use different (better) method for establishing link between AP,
            that is not compatible with APs in non-mesh mode. This method avoids one-sided
            WDS links that are created only by one of the two APs. Such links cannot pass
            any data.When AP or station is establishing WDS connection with another AP, it
            uses L(connect-list,#Connect_List) to check whether this connection is allowed. If station in *station-wds* mode is establishing connection with AP, AP uses L(access-list,#Access_List) to check whether this connection is allowed.If
            **mode** is *station-wds*, then this property has no effect.
        ...
        wireless_protocol:
          ...
          description: |
            ...
            - *unspecified* - protocol mode used on previous RouterOS versions (v3.x, v4.x).
            ...
            - *any* : on AP - regular 802.11 Access Point or Nstreme Access Point; on
              station - selects Access Point without specific sequence, it could be changed by
              connect-list rules.
            - *nstreme* - enables Nstreme protocol (the same as old enable-nstreme setting).
            - *nv2* - enables Nv2 protocol.
            - *nv2 nstreme* : on AP - uses first wireless-protocol setting, always Nv2; on
              station - searches for Nv2 Access Point, then for Nstreme Access Point.
            - *nv2 nstreme 802.11* - on AP - uses first wireless-protocol setting, always
              Nv2; on station - searches for Nv2 Access Point, then for Nstreme Access Point,
              then for regular 802.11 Access Point.
...
from ..module_utils.resources.wireless.wireless import WirelessResource
from ..module_utils.config.config import Config


def main():
    module = AnsibleModule(argument_spec=WirelessResource.argument_spec)
    result = Config(module, WirelessResource).execute_module()
